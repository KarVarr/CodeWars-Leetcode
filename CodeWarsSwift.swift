...
func toNato(_ words: String) -> String {
  ...
  let str = Array(words).map{$0.uppercased()}
  ...
  let nato: [Character: String] = [
   ...
   for word in str {
     result.append(nato[Character(word)] ?? "")
   }
  
  return result.map{String($0)}.joined(separator: "").trimmingCharacters(in: .whitespacesAndNewlines)
}
